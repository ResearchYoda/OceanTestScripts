"""
Test script for ProfilingSonar.

Sensor: ProfilingSonar
Output: 2D array of shape (AzimuthBins, RangeBins) = (512, 512).
  Each element is acoustic return intensity for that azimuth/range cell.
  The profiling sonar has a narrow elevation angle (1 deg), acting like
  a fan-beam that profiles the cross-section of the water column.

Config:
  RangeBins=512, AzimuthBins=512
  RangeMin=1 m, RangeMax=60 m
  Elevation=1 deg, Azimuth=120 deg
  InitOctreeRange=70
  AddSigma=0.15, MultSigma=0.2, RangeSigma=0.1
  AzimuthStreaks=-1, ScaleNoise=True, MultiPath=True
Socket: SonarSocket

Display: polar fan image (IMG_W=700, IMG_H=600).
Polar-to-pixel lookup is precomputed once outside the loop using numpy vectorised ops.
COLORMAP_HOT applied. Range arc labels every 10 m.
Press 'q' to quit.
...
def draw_range_arcs(canvas: np.ndarray, interval: float = 10.0) -> np.ndarray:
...
if __name__ == "__main__":
    ...
    with holoocean.environments.HoloOceanEnvironment(
        ...
        for step in range(3000):
            ...
            sonar = state.get("ProfilingSonar", None)
            if sonar is not None:
                ...
                coloured = draw_range_arcs(coloured, interval=10.0)

                cv2.imshow("ProfilingSonar", coloured)
            ...
            if step % 50 == 0:
                ...
                print(f"Step {step:4d} | ProfilingSonar peak = {peak:.4f}")
